# - - - - - - - - - - - - - - - - - - - - - - - - - #
#  COVID-19 Canada Data Bot Created By: Alyx#9463   #
# - - - - - - - - - - - - - - - - - - - - - - - - - #
...
version = 'v0.6 Beta'
TOKEN = 'YOURTOKENHERE' 
admin = [181104180570750978] # Alyx#9463 userID
...
async def scheduler(): # scheduler functions loops every 60 seconds and checks if time == 11:59. if it passes, send canada and ontario covid-19 summary data
    if datetime.now(est).hour == 23 and datetime.now().minute == 59:
        ...
        channel = bot.get_channel(801660645291851786) # #covid-19-data channel
        zerog_summary = bot.get_channel(813876384236699648) # 0g #covid-19-data channel
        await channel.send(embed=embed_cad) # sends canada summary
        await channel.send(embed=embed_ont) # sends ontario summary
        await zerog_summary.send(embed=embed_cad) 
        await zerog_summary.send(embed=embed_ont)
